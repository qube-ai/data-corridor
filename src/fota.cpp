...
void setupForOTA(String ssid, String pass) {
    Serial.println();
    Serial.println();
    Serial.println();
...
void fota::performOTAUpdate(String version, String ssid, String pass) {
    Serial.println("Setting up for an OTA update");
    setupForOTA(ssid, pass);
    ...
    if ((WiFiMulti.run() == WL_CONNECTED)) {
        ...
        // String checksumRelativeURL = "/ota-server/" + version +
        // "/checksum.txt";
        Serial.println("Firmware relative download URL: " +
                       firmwareRelativeURL);
        ...
        t_httpUpdate_return ret =
            ESPhttpUpdate.update(client, host, 80, firmwareRelativeURL);
        // t_httpUpdate_return ret = ESPhttpUpdate.update(client, finalURL);
        // t_httpUpdate_return ret = ESPhttpUpdate.update(host, 80,
        // firmwareRelativeURL);
